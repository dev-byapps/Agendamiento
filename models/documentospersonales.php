...
class Documentospersonales extends Conectar
{
    //----------CATEGORIAS------------
    public function buscarcategoriasper()
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "call SP_Buscar_catdocper()";
    ...
    public function crearcategoria($nombre)
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "call SP_Crear_categoriadocper(?)";
        $sql = $conectar->prepare($sql);
    ...
    public function guardar_titulo($id_cat, $titulo)
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "call SP_Editar_titulocatdocper(?,?)";
        $sql = $conectar->prepare($sql);
    ...
    public function eliminarcategoria($idcat)
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "call SP_Eliminar_categoriadoper(?)";
        $sql = $conectar->prepare($sql);
    ...
    //--------------------------------
    //----------DOCUMENTOS------------
    public function documentosxidcat($id_cat, $label, $usu, $perfil)
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "CALL SP_Buscar_docper_idcat(?,?,?,?)";
        $sql = $conectar->prepare($sql);
        $sql->bindValue(1, $id_cat, PDO::PARAM_INT);
    ...
    public function insert_docpersonal($docnom, $cat, $cat_est, $usu_id, $creado)
    {
        ...
        $url_insert = dirname(__FILE__) . "/../documents/docpersonales/" . $usu_id; //Carpeta donde subiremos nuestros archivos
        ...
        if (move_uploaded_file($url_temp, $url_target)) {
            ...
            $sql->bindValue(6, $creado, PDO::PARAM_INT);
            $sql->execute();
            $sql->fetchAll();
            unset($conectar);
        } else {
    ...
    public function update_docpersonal($docid, $docnom, $cat, $cat_est)
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "call SP_Editar_docperxid(?,?,?,?)";
        $sql = $conectar->prepare($sql);
    ...
    public function cambiarestadoeliminado($docid)
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "call SP_Editar_docper_estadoeliminado(?)";
        $sql = $conectar->prepare($sql);
